...
- **資產清單** — **程式自動彙總**，不讓模型寫：場景/角色的出現集與次數、一次性場景的複用方案、生成難點預警、**角色資產量折算**（主角組出全套設定圖、重要配角出半身參考、功能性角色提示詞直出）
...
- **品質門**：頁首徽章 + 未過時的病灶橫幅 + 文末完整清單，✓/✗ 由程式算好烘進頁面
- **匯出 JSON** 按鈕：下載的就是 `outline.json` 原樣，改完能直接重新匯入 `render` / `validate`
- 全部圖形是內聯 SVG/CSS，配色跑過視覺化驗證器，零外部相依，離線雙擊能開
...
80 萬字塞不進上下文。`chunk` 按章節標題分卷（預設每卷 15 章），每卷併發出中間摘要再彙總；辨識不出章節就按字數切。上限 60 卷，超了明確報 `truncated`，**不靜默截斷**。
...
node scripts/novel-outline.mjs render outline.json --html       # 出報告
node scripts/novel-outline.mjs assets outline.json              # 資產清單 JSON
```

## 跟 novel-characters 的關係

分工：**novel-characters 管角色設定**（側寫/形象提示詞/音色/設定圖），**novel-outline 管改編結構**（砍線/合人/排爽點/分集）。有 `cast.json` 就直接當人物原料提供進來，不用重拆原文。本 skill 不寫臺詞、不做分鏡、不出提示詞。
...
200 項斷言，覆蓋分卷 / 校驗 / 品質門逐項擊穿 / 資產彙總 / 渲染 / 匯出。不調模型、不花額度、1 秒跑完。改完程式先跑這個。
